...
Версия 1.4.0 добавляет **расширенные возможности поиска**, **анализ данных** и **исправления критических ошибок**.
...
2. **🌐 Три новых API Endpoint для аналитики**
...
# Применение фильтра
filtered = AdvancedSearchEngine.filter_properties(properties, filter)

# Удаление дубликатов
unique = AdvancedSearchEngine.deduplicate_properties(filtered)

# Сортировка
ranked = AdvancedSearchEngine.rank_properties(unique, sort_by="price_per_area")

# Анализ распределения
distribution = AdvancedSearchEngine.get_price_distribution(properties)
```
